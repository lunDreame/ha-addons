...
KOCOM_LIGHT_SIZE = {'livingroom': 3, 'bedroom': 2,
                    'room1': 2, 'room2': 2, 'kitchen': 3}
KOCOM_PLUG_SIZE = {'livingroom': 2, 'bedroom': 2,
                   'room1': 2, 'room2': 2, 'kitchen': 2}
...
KOCOM_ROOM = {'00': 'livingroom', '01': 'bedroom',
              '02': 'room2', '03': 'room1', '04': 'kitchen'}
KOCOM_ROOM_THERMOSTAT = {'00': 'livingroom',
...
# KOCOM 코콤 패킷 기본정보
KOCOM_DEVICE = {'01': DEVICE_WALLPAD, '0e': DEVICE_LIGHT, '36': DEVICE_THERMOSTAT,
                '3b': DEVICE_PLUG, '44': DEVICE_ELEVATOR, '2c': DEVICE_GAS, '48': DEVICE_FAN, '98': DEVICE_AIR}
...
class rs485:
    ...
    @property
    def _wp_air(self):
        return True if self._wp_list['air'] == 'True' else False
...
class Kocom(rs485):
    def __init__(self, client, name, device, packet_len):
        ...
        self.wp_air = self.client._wp_air
        ...
        for d_name in KOCOM_DEVICE.values():
            if d_name == DEVICE_ELEVATOR or d_name == DEVICE_GAS:
            ...
            elif d_name == DEVICE_AIR:
                self.wp_list[d_name] = {}
                self.wp_list[d_name][DEVICE_WALLPAD] = {
                    'scan': {'tick': 0, 'count': 0, 'last': 0}}
                self.wp_list[d_name][DEVICE_WALLPAD][d_name] = {
                    'state': 0, 'last': 'state', 'count': 0}
                ...
                for r_name in KOCOM_ROOM_THERMOSTAT.values():
                    ...
                    self.wp_list[d_name][r_name]['current_temp'] = {
                        'state': 0, 'set': 0, 'last': 'state', 'count': 0}
                    self.wp_list[d_name][r_name]['target_temp'] = {
                        'state': INIT_TEMP, 'set': INIT_TEMP, 'last': 'state', 'count': 0}
                ...
                for r_name in KOCOM_ROOM.values():
                    ...
                    if d_name == DEVICE_LIGHT:
                        for i in range(0, KOCOM_LIGHT_SIZE[r_name] + 1):
                            self.wp_list[d_name][r_name][d_name + str(
                                i)] = {'state': 'off', 'set': 'off', 'last': 'state', 'count': 0}
                    if d_name == DEVICE_PLUG:
                        for i in range(0, KOCOM_PLUG_SIZE[r_name] + 1):
                            self.wp_list[d_name][r_name][d_name + str(
                                i)] = {'state': 'on', 'set': 'on', 'last': 'state', 'count': 0}
        ...
        self._t1 = threading.Thread(
            target=self.get_serial, args=(name, packet_len))
        self._t1.start()
    ...
    def connect_mqtt(self, server, name):
        ...
        if server['anonymous'] != 'True':
            if server['server'] == '' or server['username'] == '' or server['password'] == '':
                logger.info('{} 설정을 확인하세요. Server[{}] ID[{}] PW[{}] Device[{}]'.format(
                    CONF_MQTT, server['server'], server['username'], server['password'], name))
                return False
            mqtt_client.username_pw_set(
                username=server['username'], password=server['password'])
            logger.debug('{} STATUS. Server[{}] ID[{}] PW[{}] Device[{}]'.format(
                CONF_MQTT, server['server'], server['username'], server['password'], name))
        else:
            logger.debug('{} STATUS. Server[{}] Device[{}]'.format(
    ...
    def on_message(self, client, obj, msg):
        ...
        if 'config' in _topic and _topic[0] == 'rs485' and _topic[1] == 'bridge' and _topic[2] == 'config':
            if _topic[3] == 'log_level':
                ...
                logger.info('[From HA]{} = {}({})'.format(
                    _payload, chksum[0], chksum[1]))
        ...
        if device == HA_LIGHT or device == HA_SWITCH:
            ...
            try:
                if device == DEVICE_GAS:
                    ...
                    self.wp_list[device][room][sub_device][command] = payload
                    self.wp_list[device][room][sub_device]['last'] = command
                logger.info('[From HA]{}/{}/{}/{} = {}'.format(device,
                            room, sub_device, command, payload))
            except:
            ...
            try:
                ...
                logger.info('[From HA]{}/{}/set = [mode={}, target_temp={}]'.format(device, room,
                            self.wp_list[device][room]['mode']['set'], self.wp_list[device][room]['target_temp']['set']))
            ...
            try:
                ...
                logger.info('[From HA]{}/{}/set = [mode={}, speed={}]'.format(device, room,
                            self.wp_list[device][room]['mode']['set'], self.wp_list[device][room]['speed']['set']))
                self.send_to_homeassistant(device, room, ha_payload)
            except:
                logger.info('[From HA]Error {} = {}'.format(topic, payload))
    ...
    def homeassistant_device_discovery(self, initial=False, remove=False):
        ...
        if self.wp_elevator:
            ...
            ha_payload = {
                'name': '{}_{}'.format(self._name, DEVICE_ELEVATOR),
                ...
                'pl_on': 'on',
                'pl_off': 'off',
                'uniq_id': '{}_{}'.format(self._name, DEVICE_ELEVATOR),
                'device': {
                    'ids': 'kocom_wallpad',
                    'name': 'kocom_wallpad',
                    'mf': 'KOCOM',
        ...
        if self.wp_air:
            ha_topic = '{}/{}/{}_{}/config'.format(
                HA_PREFIX, HA_SENSOR, DEVICE_AIR, self.kcAirRoom)
            ha_payload = {
                'name': '{}_{}_{}_state'.format(self._name, DEVICE_AIR, self.kcAirRoom),
                'stat_t': '{}/{}/{}_{}/state'.format(HA_PREFIX, HA_SENSOR, DEVICE_AIR, self.kcAirRoom),
                'val_tpl': '{{ value_json.' + self.kcAirRoom + ' }}',
                'ic': f'mdi:{self.kcAirIcon}',
                'name': '{}_{}_{}_state'.format(self._name, DEVICE_AIR, self.kcAirRoom),
                'device': {
                    'ids': 'kocom_wallpad',
                    'name': 'kocom_wallpad',
        ...
        if self.wp_gas:
            ...
            ha_payload = {
                'name': '{}_{}'.format(self._name, DEVICE_GAS),
                ...
                'pl_on': 'on',
                'pl_off': 'off',
                'uniq_id': '{}_{}'.format(self._name, DEVICE_GAS),
                'device': {
                    'ids': 'kocom_wallpad',
                    'name': 'kocom_wallpad',
                    'mf': 'KOCOM',
            ...
            ha_payload = {
                'name': '{}_{}_state'.format(self._name, DEVICE_GAS),
                ...
                if type(r_value) == dict:
                    for sub_device, d_value in r_value.items():
                        if type(d_value) == dict:
                            ...
                            ha_payload = {
                                'name': '{}_light_{}_{}'.format(self._name, room[:-1], sub_device[:-1]),
                            ...
                            if remove:
                                ...
                                publish_list.append(
                                    {ha_topic: json.dumps(ha_payload)})
        if self.wp_plug:
                ...
                if type(r_value) == dict:
                    for sub_device, d_value in r_value.items():
                        if type(d_value) == dict:
                            ...
                            ha_payload = {
                                'name': '{}_plug_{}_{}'.format(self._name, room[:-1], sub_device[:-1]),
        ...
        if self.wp_thermostat:
            for room, r_list in self.wp_list[DEVICE_THERMOSTAT].items():
                if type(r_list) == dict:
                    ...
                    ha_payload = {
                        'name': '{}_{}_{}'.format(self._name, DEVICE_THERMOSTAT, room[:-1]),
    ...
    def send_to_homeassistant(self, device, room, value):
        ...
        if device == DEVICE_LIGHT:
            ...
            logger.info(
                "[To HA]{}/{}/{}/state = {}".format(HA_PREFIX, HA_CLIMATE, room, v_value))
        elif device == DEVICE_AIR:
            self.d_mqtt.publish(
                "{}/{}/{}_{}/state".format(HA_PREFIX, HA_SENSOR, DEVICE_AIR, room), v_value)
            logger.info("[To HA]{}/{}/{}_{}/state = {}".format(HA_PREFIX,
                        HA_SENSOR, DEVICE_AIR, room, v_value))
        elif device == DEVICE_ELEVATOR:
            ...
            v_value = json.dumps({device: value})
            self.d_mqtt.publish(
                "{}/{}/{}_{}/state".format(HA_PREFIX, HA_SENSOR, room, DEVICE_GAS), v_value)
            logger.info("[To HA]{}/{}/{}_{}/state = {}".format(HA_PREFIX,
                        HA_SENSOR, room, DEVICE_GAS, v_value))
            self.d_mqtt.publish(
                "{}/{}/{}_{}/state".format(HA_PREFIX, HA_SWITCH, room, DEVICE_GAS), v_value)
            logger.info("[To HA]{}/{}/{}_{}/state = {}".format(HA_PREFIX,
                        HA_SWITCH, room, DEVICE_GAS, v_value))
    ...
    def value_packet(self, p):
        ...
        try:
            ...
            v['src_room'] = KOCOM_ROOM.get(
                p['src_room']) if v['src_device'] != DEVICE_THERMOSTAT else KOCOM_ROOM_THERMOSTAT.get(p['src_room'])
            v['dst_device'] = KOCOM_DEVICE.get(p['dst_device'])
            v['dst_room'] = KOCOM_ROOM.get(
                p['dst_room']) if v['src_device'] != DEVICE_THERMOSTAT else KOCOM_ROOM_THERMOSTAT.get(p['dst_room'])
            v['value'] = p['value']
            if v['src_device'] == DEVICE_FAN:
                v['value'] = self.parse_fan(p['value'])
            elif v['src_device'] == DEVICE_AIR:
                v['value'] = self.parse_air(p['value'])
                ...
                v['value'] = self.parse_thermostat(
                    p['value'], self.wp_list[v['src_device']][v['src_room']]['target_temp']['state'])
    ...
    def packet_parsing(self, packet, name='kocom', from_to='From'):
        ...
        try:
            if v['command'] == "조회" and v['src_device'] == DEVICE_WALLPAD:
                if name == 'HA':
                    self.write(self.make_packet(
                        v['dst_device'], v['dst_room'], '조회', '', ''))
                logger.debug('[{} {}]{}({}) {}({}) -> {}({})'.format(from_to, name, v['type'],
                             v['command'], v['src_device'], v['src_room'], v['dst_device'], v['dst_room']))
            else:
                logger.debug('[{} {}]{}({}) {}({}) -> {}({}) = {}'.format(from_to, name, v['type'],
                             v['command'], v['src_device'], v['src_room'], v['dst_device'], v['dst_room'], v['value']))
            ...
            if (v['type'] == 'ack' and v['dst_device'] == DEVICE_WALLPAD) or (v['type'] == 'send' and v['dst_device'] == DEVICE_ELEVATOR):
                ...
                elif v['src_device'] == DEVICE_FAN or v['src_device'] == DEVICE_GAS or v['src_device'] == DEVICE_AIR:
    ...
    def set_list(self, device, room, value, name='kocom'):
        try:
            logger.info(
                '[From {}]{}/{}/state = {}'.format(name, device, room, value))
            ...
            if device == DEVICE_GAS or device == DEVICE_ELEVATOR or device == DEVICE_AIR:
                ...
                for sub, v in value.items():
                    try:
                        ...
                        logger.info(
                            '[From {}]Error SetListDevice {}/{}/{}/state = {}'.format(name, device, room, sub, v))
                ...
                for sub, v in value.items():
                    try:
                        if sub == 'mode':
                            ...
                            self.wp_list[device][room][sub]['state'] = int(
                                float(v))
                ...
                if now - self.tick > KOCOM_INTERVAL / 1000:
                    try:
                        for device, d_list in self.wp_list.items():
                            if type(d_list) == dict and ((device == DEVICE_ELEVATOR and self.wp_elevator) or (device == DEVICE_FAN and self.wp_fan) or (device == DEVICE_AIR and self.wp_air) or (device == DEVICE_GAS and self.wp_gas) or (device == DEVICE_LIGHT and self.wp_light) or (device == DEVICE_PLUG and self.wp_plug) or (device == DEVICE_THERMOSTAT and self.wp_thermostat)):
                                            ...
                                            for sub_d, sub_v in r_list.items():
                                                if sub_d != 'scan':
                                                    if sub_v['count'] > 4:
                                                        ...
                                                        sub_v['last'] = now
                                                        if device == DEVICE_GAS or device == DEVICE_AIR:
                                                            sub_v['last'] += 5
                                                        elif device == DEVICE_ELEVATOR:
                                                            sub_v['last'] = 'state'
                                                        self.set_serial(
                                                            device, room, sub_d, sub_v['set'])
    ...
    def set_serial(self, device, room, target, value, cmd='상태'):
        if (time.time() - self.tick) < KOCOM_INTERVAL / 1000:
            return
        if cmd == '상태':
            logger.info('[To {}]{}/{}/{} -> {}'.format(self._name,
                        device, room, target, value))
        elif cmd == '조회':
            logger.info('[To {}]{}/{} -> 조회'.format(self._name, device, room))
        packet = self.make_packet(device, room, '상태', target, value) if cmd == '상태' else self.make_packet(
            device, room, '조회', '', '')
        v = self.value_packet(self.parse_packet(packet))

        logger.debug('[To {}]{}'.format(self._name, packet))
        if v['command'] == "조회" and v['src_device'] == DEVICE_WALLPAD:
            logger.debug('[To {}]{}({}) {}({}) -> {}({})'.format(self._name, v['type'],
                         v['command'], v['src_device'], v['src_room'], v['dst_device'], v['dst_room']))
        else:
            logger.debug('[To {}]{}({}) {}({}) -> {}({}) = {}'.format(self._name, v['type'], v['command'],
                         v['src_device'], v['src_room'], v['dst_device'], v['dst_room'], v['value']))
    ...
    def make_packet(self, device, room, cmd, target, value):
        ...
        p_room = KOCOM_ROOM_REV.get(
            room) if device != DEVICE_THERMOSTAT else KOCOM_ROOM_THERMOSTAT_REV.get(room)
        p_dst = KOCOM_DEVICE_REV.get(
            DEVICE_WALLPAD) + KOCOM_ROOM_REV.get(DEVICE_WALLPAD)
        ...
        if cmd == '조회':
            ...
            if device == DEVICE_ELEVATOR:
                ...
                p_dst = KOCOM_DEVICE_REV.get(
                    device) + KOCOM_ROOM_REV.get(DEVICE_WALLPAD)
                ...
                try:
                        ...
                        if target != sub_device:
                            if target == all_device:
                                if sub_device in self.wp_list[device][room]:
                                    p_value += 'ff' if value == 'on' else str(
                                        '00')
                                else:
                                    p_value += '00'
                            else:
                    ...
                    logger.debug(
                        '[Make Packet] Error on DEVICE_LIGHT or DEVICE_PLUG')
            elif device == DEVICE_THERMOSTAT:
                try:
                    mode = self.wp_list[device][room]['mode']['set']
                    target_temp = self.wp_list[device][room]['target_temp']['set']
    ...
    def parse_air(self, value='0000000000000000'):
        air = {
            "pm10": int(value[:2], 16),
    ...
    def parse_switch(self, device, room, value='0000000000000000'):
        switch = {}
        on_count = 0
        to_i = KOCOM_LIGHT_SIZE.get(
            room) + 1 if device == DEVICE_LIGHT else KOCOM_PLUG_SIZE.get(room) + 1
        for i in range(1, to_i):
            switch[device + str(i)] = 'off' if value[i *
                                                     2-2:i*2] == '00' else 'on'
    ...
    def parse_thermostat(self, value='0000000000000000', init_temp=False):
        ...
        if heat_mode == 'heat' and away_mode == 'on':
            ...
            thermo['target_temp'] = INIT_TEMP if not init_temp else int(
                init_temp)
...
class Grex:
    def __init__(self, client, cont, vent):
        ...
        _t4 = threading.Thread(target=self.get_serial, args=(
            self.contoller['serial'], self.contoller['name'], self.contoller['length']))
        _t4.daemon = True
        _t4.start()
        _t5 = threading.Thread(target=self.get_serial, args=(
            self.ventilator['serial'], self.ventilator['name'], self.ventilator['length']))
        _t5.daemon = True
        _t5.start()
    ...
    def homeassistant_device_discovery(self, initial=False):
        ...
        subscribe_list.append(('rs485/bridge/#', 0))
        ha_topic = '{}/{}/{}_{}/config'.format(
            HA_PREFIX, HA_FAN, 'grex', DEVICE_FAN)
        ha_payload = {
            'name': '{}_{}'.format(self._name, DEVICE_FAN),
    ...
    def send_to_homeassistant(self, target, value):
        if target == HA_FAN:
            ...
            logger.info("[To HA]{}/{}/{}/state = {}".format(HA_PREFIX,
                        HA_FAN, 'grex', json.dumps(value)))
        elif target == HA_SENSOR:
            self.d_mqtt.publish("{}/{}/{}_{}/state".format(HA_PREFIX, HA_SENSOR,
                                'grex', DEVICE_FAN), json.dumps(value, ensure_ascii=False))
            logger.info("[To HA]{}/{}/{}_{}/state = {}".format(HA_PREFIX, HA_SENSOR,
                        'grex', DEVICE_FAN, json.dumps(value, ensure_ascii=False)))
    ...
    def packet_parsing(self, packet, packet_name):
        ...
        if p_prefix == 'd00a':
            ...
            if self.grex_cont['mode'] != GREX_MODE[p_mode] or self.grex_cont['speed'] != GREX_SPEED[p_speed]:
                ...
                logger.info('[From {}]mode:{} / speed:{}'.format(packet_name,
            ...
            if response_packet != '':
                self.contoller['serial'].write(
                    bytearray.fromhex(response_packet))
                # logger.debug("[Tooo grex_controller]{}".format(response_packet))
            if control_packet != '':
                self.ventilator['serial'].write(
                    bytearray.fromhex(control_packet))
            ...
            if self.vent_cont['speed'] != GREX_SPEED[p_speed]:
                ...
                logger.info('[From {}]speed:{}'.format(
                    packet_name, self.vent_cont['speed']))
                ...
                if r._connect[device].isOpen():
                    ...
                    try:
                        ...
                        if _name == 'kocom':
                            ...
                            _grex_ventilator = {
                                'serial': r._connect[device], 'name': _name, 'length': 12}
                        elif _name == 'grex_controller':
                            _grex_controller = {
                                'serial': r._connect[device], 'name': _name, 'length': 11}
                    except:
